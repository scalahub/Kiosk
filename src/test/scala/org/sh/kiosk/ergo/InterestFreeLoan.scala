...
object InterestFreeLoan extends App {
  ...
  $ergoScript.$myEnv.setCollByte("usdTokenID", $usdTokenID)
  ...
  $ergoScript.$myEnv.setInt("emi", 1000) // equal monthly installment in USD cents
  ...
  val ergoScript =
    """{
      ...
      |
      |  correctTx && ((proveDlog(alice) && correctBobBox) || (proveDlog(bob) && nonPayment))
      |
      |}""".stripMargin
