...
object Compiler {
  ...
  private def declareProtocol(p: Protocol): Seq[Declaration] = {
    optSeq(p.constants) ++ optSeq(p.unaryOps) ++ optSeq(p.binaryOps) ++ optSeq(p.conversions) ++
      optSeq(p.dataInputs).flatMap(declareInput) ++
      optSeq(p.inputs).flatMap(declareInput) ++
      optSeq(p.outputs).flatMap(declareOutput) ++
      p.fee.toSeq
  }

  private def declareInput(input: Input) = {
    opt(input.boxId) ++ opt(input.address) ++ optSeq(input.registers) ++
      optSeq(input.tokens).flatMap(declareToken) ++
      input.nanoErgs.toSeq
  }

  private def declareOutput(output: Output) = {
    Seq(output.address) ++ optSeq(output.registers) ++
      optSeq(output.tokens).flatMap(declareToken) ++
      Seq(output.nanoErgs)
  }
